...
export default function FavouritePage() {
  ...
  const uniqueStores = Array.from(
    new Set(favorites.map((item: any) => item.shop_name).filter(Boolean))
  );
  ...
  const filteredAndSortedFavorites = favorites
    .filter((item: any) => {
      ...
      const matchesFilter = filterBy === "all" || item.shop_name === filterBy;
  ...
  const handleShare = (item: any) => {
    if (navigator.share) {
      navigator.share({
        title: item.name,
        text: `Check out this ${item.name} for ₪${
  ...
  const addAllToCart = () => {
    filteredAndSortedFavorites.forEach((item: any) => {
  ...
  const clearAllFavorites = () => {
    if (
      ...
      favorites.forEach((item: any) => removeFromFavorites(item.id));
    }
  };
          ...
          <>
            ...
            {filteredAndSortedFavorites.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredAndSortedFavorites.map((item: any) => (
                  <Card
                    key={item.id}
                          ...
                          <div className="flex items-center justify-between">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-lg font-bold text-gray-900 dark:text-white">
                                  ₪
                                  {(item.discount_price ?? item.price).toFixed(
                                    2
                                  )}
                                </span>
                              </div>
